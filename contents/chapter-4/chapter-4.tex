...
\label{sec:kondisi_dasar_sistem}
...
Subbab ini membahas respons relay pada kondisi dasar tanpa integrasi IBR ketika terjadi gangguan internal \textit{double line to ground} (DLG) pada saluran TL89. Skenario ini digunakan sebagai acuan awal karena gangguan berada di dalam daerah proteksi saluran, sehingga relay pada kedua ujung saluran diharapkan mampu mendeteksi gangguan dan memberikan perintah trip sesuai setting konvensional.

Gangguan DLG melibatkan dua fasa dan tanah sehingga respons relay diamati melalui beberapa elemen pengukuran. Pada relay 21, evaluasi dilakukan berdasarkan posisi impedansi tampak elemen fasa-tanah dan fasa-fasa pada sisi Bus 8 dan Bus 9 terhadap zona proteksi. Pada relay 87L, evaluasi dilakukan berdasarkan posisi arus diferensial dan arus penahan tiap fasa terhadap batas operasi relay. Hasil verifikasi ini menjadi pembanding untuk menilai perubahan respons relay pada skenario sistem yang telah terintegrasi IBR.
...
\subsection{Respons Relay terhadap gangguan External Three Phase}
\label{subsec:tanpa_IBR_ex_3p}

Subbab ini membahas respons relay pada kondisi dasar tanpa integrasi IBR ketika terjadi gangguan eksternal tiga fasa. Berbeda dengan gangguan internal pada Subbab \ref{subsec:tanpa_IBR_in_dlg}, gangguan pada skenario ini berada di luar daerah proteksi utama saluran TL89. Oleh karena itu, bagian ini digunakan untuk memeriksa kemampuan relay dalam mempertahankan selektivitas ketika arus gangguan besar mengalir melalui sistem, tetapi titik gangguan tidak berada pada saluran yang diamankan.

Gangguan tiga fasa bersifat simetris sehingga evaluasi relay 21 difokuskan pada elemen fasa-fasa, yaitu ZAB, ZBC, dan ZCA, serta pembacaan arus sekunder CT yang menentukan kondisi operasi relay. Untuk relay 87L, evaluasi dilakukan melalui posisi arus diferensial dan arus penahan pada masing-masing fasa terhadap batas operasi. Hasil pada bagian ini menjadi acuan untuk menilai apakah skema proteksi tetap mampu membedakan gangguan eksternal sebelum sistem dibandingkan dengan kondisi yang telah terintegrasi IBR.
...
Subbab ini membahas respons relay pada kondisi dasar tanpa integrasi IBR ketika terjadi gangguan eksternal \textit{single line to ground} (SLG). Gangguan ini berada di luar daerah proteksi utama saluran TL89, sehingga relay yang diamati seharusnya mampu menahan operasi untuk gangguan yang tidak terjadi pada saluran yang diamankan. Berbeda dengan gangguan tiga fasa, gangguan SLG bersifat tidak seimbang dan melibatkan jalur tanah, sehingga respons relay perlu diamati melalui elemen fasa-tanah dan perubahan arus pada fasa yang terganggu.

Pada relay 21, evaluasi difokuskan pada elemen ZAG di sisi Bus 8 dan Bus 9 serta pembacaan arus sekunder CT yang memengaruhi kondisi operasi relay. Pada relay 87L, evaluasi dilakukan dengan melihat posisi arus diferensial dan arus penahan pada setiap fasa terhadap batas operasi. Hasil pada bagian ini digunakan sebagai acuan untuk menilai apakah relay tetap selektif terhadap gangguan eksternal SLG sebelum responsnya dibandingkan dengan sistem yang telah terintegrasi IBR.
...
Subbab ini membahas respons relay pada kondisi dasar tanpa integrasi IBR ketika terjadi gangguan eksternal \textit{double line to ground} (DLG). Gangguan ini berada di luar daerah proteksi utama saluran TL89, sehingga hasilnya digunakan untuk memeriksa selektivitas relay terhadap gangguan tak seimbang yang melibatkan dua fasa dan jalur tanah. Kondisi ini penting karena gangguan DLG dapat memengaruhi pembacaan elemen fasa-tanah maupun fasa-fasa pada relay jarak.

Pada relay 21, evaluasi dilakukan melalui elemen ZAG, ZBG, dan ZAB di sisi Bus 8 serta Bus 9, disertai pembacaan arus sekunder CT sebagai dasar untuk menilai kondisi operasi relay. Pada relay 87L, evaluasi dilakukan berdasarkan posisi arus diferensial dan arus penahan tiap fasa terhadap batas operasi. Hasil pada bagian ini menjadi acuan untuk menilai apakah relay tetap mampu membedakan gangguan eksternal DLG sebelum responsnya dibandingkan dengan sistem yang telah terintegrasi IBR.
...
\label{sec:perbandingan_kontribusi_SC_IBR}
...
Pembahasan model \textit{wind turbine type 3} diawali dengan meninjau pengaturan hubung singkat yang digunakan dalam simulasi. WT Type 3 menggunakan \textit{doubly fed asynchronous generator} sehingga karakter kontribusi arus gangguannya ditentukan oleh parameter hubung singkat pada model, termasuk besar arus awal, faktor puncak, dan kontribusi arus keadaan tunak.
...
Gambar \ref{fig:SC_Setting_WTT3} menunjukkan pengaturan hubung singkat WT Type 3 pada tab VDE/IEC. Model menggunakan tipe unit \textit{doubly fed asynchronous generator} dengan transformator unit yang dimodelkan secara eksternal. Opsi \textit{No Short-Circuit Contribution} tidak diaktifkan, sehingga WT Type 3 diperhitungkan sebagai sumber kontribusi arus hubung singkat dalam simulasi.

Pada bagian \textit{initial symmetrical short-circuit current contribution}, nilai \textit{max. instantaneous short-circuit current} ($i_{\mathrm{WDmax}}$) ditetapkan sebesar 0,15 $\mathrm{kA}$, dengan faktor $\kappa_{\mathrm{WD}}$ sebesar 1,7 dan rasio $R_{\mathrm{WD}}/X_{\mathrm{WD}}$ sebesar 0,1. Parameter tersebut menunjukkan bahwa WT Type 3 memberikan kontribusi pada periode awal gangguan. Sementara itu, arus maksimum dan minimum pada bagian \textit{steady-state short-circuit current contribution} bernilai 0 $\mathrm{kA}$, sehingga model tidak mempertahankan kontribusi arus hubung singkat pada keadaan tunak.

Nilai resistansi dan reaktansi urutan negatif, masing-masing $r_2$ dan $x_2$, ditetapkan sebesar 9999 p.u. Nilai impedansi yang sangat besar ini membatasi kontribusi arus urutan negatif dari WT Type 3. Dengan demikian, pengaturan pada Gambar \ref{fig:SC_Setting_WTT3} menggambarkan kontribusi arus gangguan yang muncul pada periode awal, tidak berlanjut pada keadaan tunak, dan sangat terbatas untuk komponen urutan negatif.
...
Berdasarkan hasil simulasi pada Gambar \ref{fig:kontribusi_WTType3}, kontribusi hubung singkat dari \textit{wind turbine type 3} terbaca sebesar 119,7 MVA atau sekitar 1,2 p.u. pada basis daya 100 MVA. Hasil ini selaras dengan pengaturan pada Gambar \ref{fig:SC_Setting_WTT3}, khususnya nilai $i_{\mathrm{WDmax}}$ sebesar 0,15 $\mathrm{kA}$ yang mengaktifkan kontribusi arus pada periode awal gangguan.

Selain nilai daya hubung singkat tersebut, Gambar \ref{fig:kontribusi_WTType3} menunjukkan bahwa kontribusi arus hubung singkat mencapai nilai puncak sebesar 245,444 $\mathrm{kA}$ atau setara dengan 3,05 p.u. Setelah periode puncak gangguan, kontribusi arus turun menjadi 100,132 $\mathrm{kA}$ atau setara dengan 1,24 p.u. Penurunan ini menunjukkan bahwa kontribusi WT Type 3 paling besar terjadi pada periode awal gangguan, kemudian berkurang setelah kondisi transien awal terlewati.

Implikasinya, kontribusi awal WT Type 3 perlu diperhitungkan ketika menganalisis pembacaan impedansi tampak, arus diferensial, arus \textit{restraint}, dan keputusan operasi relay sesaat setelah gangguan. Perbedaan antara nilai puncak dan nilai setelah gangguan menunjukkan bahwa besaran arus yang diterima relay berubah sepanjang periode gangguan. Respons relay pada bagian berikutnya tetap perlu dibaca bersama konfigurasi jaringan, lokasi gangguan, serta kontribusi sumber lain yang masih aktif.

\subsection{Wind Turbine Type 4}

Pembahasan pada model \textit{wind turbine type 4} diawali dari pengaturan hubung singkat yang digunakan pada simulasi. Model WT Type 4 juga memiliki parameter kontribusi arus gangguan yang aktif, tetapi karakter dan besar parameternya berbeda dari WT Type 3. Oleh karena itu, pengaturan pada menu hubung singkat perlu dicermati sebelum hasil kontribusi pada sistem dianalisis.
...
Berdasarkan hasil simulasi pada Gambar \ref{fig:kontribusi_WTType4}, \textit{wind turbine type 4} memberikan kontribusi daya hubung singkat sebesar 110 MVA atau setara dengan 1,1 p.u. Hasil ini konsisten dengan pengaturan pada Gambar \ref{fig:SC_Setting_WTT4}, karena opsi kontribusi hubung singkat tidak dinonaktifkan dan model masih memiliki batas arus gangguan yang didefinisikan.

Selain dalam satuan daya, Gambar \ref{fig:kontribusi_WTType4} menunjukkan bahwa kontribusi arus hubung singkat mencapai nilai puncak sebesar 130,149 $\mathrm{kA}$ atau setara dengan 1,6 p.u. Setelah periode puncak gangguan, kontribusi arus turun menjadi 92,029 $\mathrm{kA}$ atau setara dengan 1,14 p.u. Penurunan tersebut menunjukkan bahwa WT Type 4 memberikan arus terbesar pada periode awal gangguan, kemudian mempertahankan kontribusi yang lebih rendah setelah kondisi transien awal terlewati.

Implikasinya, respons proteksi pada skenario dengan WT Type 4 perlu dianalisis dengan memperhatikan perubahan arus antara periode puncak dan periode setelah gangguan. Kedua nilai tersebut dapat memengaruhi arus yang diterima relay, impedansi tampak, arus diferensial, serta arus \textit{restraint}. Hasil pada subbagian ini menjadi dasar untuk membaca perubahan respons relay pada skenario yang melibatkan WT Type 4 dan membandingkannya dengan karakter kontribusi WT Type 3 maupun PV.
...
\label{sec:dekomposisi_arus}

Bagian ini membahas dekomposisi komponen urutan arus pada saluran TL89 untuk setiap skenario gangguan yang dianalisis. Dekomposisi dilakukan dengan memisahkan arus menjadi komponen urutan positif, urutan negatif, dan urutan nol. Pemisahan ini diperlukan karena perubahan respons relay tidak hanya dipengaruhi oleh besar arus total, tetapi juga oleh pola ketidakseimbangan arus yang muncul selama gangguan.

Komponen urutan positif digunakan untuk membaca kontribusi utama arus selama gangguan, sedangkan komponen urutan negatif dan nol digunakan untuk mengidentifikasi karakter gangguan tak seimbang, khususnya gangguan \textit{single line to ground} dan \textit{double line to ground}. Hasil pada bagian ini menjadi dasar untuk menafsirkan perubahan impedansi tampak pada relay 21. Dengan demikian, pembahasan dekomposisi komponen urutan menjadi penghubung antara kontribusi arus hubung singkat pada bagian sebelumnya dan analisis operasi relay pada bagian berikutnya.

\subsection{Kondisi Tanpa Integrasi IBR}

Subbab ini menyajikan dekomposisi komponen urutan arus pada kondisi sistem tanpa integrasi IBR. Kondisi ini digunakan sebagai acuan dasar karena arus gangguan masih didominasi oleh sumber konvensional, sehingga pola komponen urutan yang terbentuk dapat digunakan untuk membaca karakter gangguan sebelum sistem dipengaruhi oleh jenis IBR tertentu. Skenario yang diamati meliputi gangguan internal DLG pada saluran TL89 serta gangguan eksternal tiga fasa, SLG, dan DLG.

Analisis pada bagian ini difokuskan pada perubahan komponen urutan positif, negatif, dan nol selama gangguan berlangsung. Untuk gangguan simetris tiga fasa, perhatian utama diberikan pada komponen urutan positif, sedangkan untuk gangguan tak seimbang seperti SLG dan DLG, komponen urutan negatif dan nol menjadi indikator penting terhadap ketidakseimbangan arus. Hasil kondisi dasar ini kemudian digunakan sebagai pembanding ketika arus pada saluran TL89 berubah akibat integrasi WT Type 3, WT Type 4, maupun \textit{photovoltaic}.
...
\label{sec:respon_relay 21_WTT3}
...
\label{sec:respon_relay 21_WTT4}
...
Subbab ini membandingkan karakteristik operasi relay 21 pada kondisi dasar tanpa IBR dan pada sistem yang diintegrasikan dengan WT Type 3, WT Type 4, serta \textit{photovoltaic}. Kondisi dasar pada Subbab \ref{sec:kondisi_dasar_sistem} digunakan sebagai acuan utama, sedangkan Subbab \ref{sec:perbandingan_kontribusi_SC_IBR} sampai \ref{sec:respon_relay 21_PV} digunakan untuk menelusuri penyebab perubahan respons melalui kontribusi arus hubung singkat, dekomposisi komponen urutan arus, dan pembacaan impedansi tampak relay.
...
Subbab ini membahas respons relay 87L pada sistem dengan \textit{wind turbine type 3} ketika terjadi gangguan internal DLG pada saluran TL89. Karena gangguan berada di dalam daerah proteksi saluran, relay 87L diharapkan menghasilkan arus diferensial yang melewati batas operasi sehingga relay dapat memberikan perintah trip. Hasil pada skenario ini dibandingkan dengan kondisi dasar tanpa IBR pada Subbab \ref{subsec:tanpa_IBR_in_dlg} untuk menilai apakah integrasi WT Type 3 mengubah kemampuan relay dalam mendeteksi gangguan internal.

Analisis dilakukan pada fasa A, fasa B, dan fasa C melalui hubungan antara arus diferensial dan arus penahan. Posisi titik arus terhadap batas operasi digunakan sebagai indikator utama keputusan relay. Dengan demikian, pembahasan pada subbab ini tidak hanya melihat apakah relay memberikan perintah trip, tetapi juga apakah karakteristik operasinya masih konsisten dengan prinsip kerja relay 87L pada gangguan internal.
...
Terlihat pada gambar \ref{fig:plot_Ax1_Ay1} arus diferensial dan arus penahan fasa A saat adanya gangguan internal DLG menunjukkan bahwa arus berada di atas batas yang ditentukan oleh setting relay. Relay mendeteksi adanya gangguan kemudian memerintahkan CB untuk trip. Relay 87L beroperasi dengan benar berdasarkan acuan pada Subbab \ref{subsec:tanpa_IBR_in_dlg}.
...
Subbab ini membahas respons relay 87L pada sistem dengan \textit{wind turbine type 3} ketika terjadi gangguan eksternal DLG. Berbeda dengan Skenario 1, titik gangguan pada skenario ini berada di luar daerah proteksi saluran TL89, sehingga relay 87L diharapkan tetap berada pada daerah penahan dan tidak memberikan perintah trip. Hasil pada skenario ini dibandingkan dengan acuan gangguan eksternal DLG tanpa IBR pada Subbab \ref{subsec:tanpa_IBR_ex_dlg} untuk menilai apakah integrasi WT Type 3 memengaruhi selektivitas relay diferensial saluran.

Analisis dilakukan pada fasa A, fasa B, dan fasa C dengan memperhatikan posisi arus diferensial dan arus penahan terhadap batas operasi relay. Jika titik arus tetap berada di bawah batas operasi, maka relay 87L masih mampu membedakan gangguan eksternal dari gangguan internal pada kondisi sistem dengan WT Type 3. Dengan demikian, pembahasan ini menjadi dasar untuk menilai konsistensi relay 87L pada skenario eksternal sebelum dibandingkan dengan jenis IBR lainnya.
...
Terlihat pada gambar \ref{fig:plot_Bx3_By3} arus diferensial dan arus penahan fasa B saat adanya gangguan external DLG menunjukkan bahwa arus berada di bawah batas yang ditentukan oleh setting relay. Relay tidak mendeteksi adanya gangguan maka relay tidak memerintahkan CB untuk trip. Relay 87L beroperasi dengan benar berdasarkan acuan pada Subbab \ref{subsec:tanpa_IBR_ex_dlg}.
...
Subbab ini membahas respons relay 87L pada sistem dengan \textit{wind turbine type 3} ketika terjadi gangguan eksternal tiga fasa. Pada skenario ini, titik gangguan berada di luar daerah proteksi saluran TL89, sehingga relay 87L diharapkan tetap berada pada daerah penahan dan tidak memberikan perintah trip. Hasil pada skenario ini dibandingkan dengan acuan gangguan eksternal tiga fasa tanpa IBR pada Subbab \ref{subsec:tanpa_IBR_ex_3p} untuk menilai apakah integrasi WT Type 3 memengaruhi selektivitas relay diferensial saluran.

Analisis dilakukan pada fasa A, fasa B, dan fasa C melalui hubungan antara arus diferensial dan arus penahan. Karena gangguan tiga fasa merupakan gangguan seimbang, pembahasan diarahkan pada kemampuan relay 87L mempertahankan titik operasi di bawah batas trip meskipun terjadi perubahan arus pada seluruh fasa. Dengan demikian, subbab ini digunakan untuk memastikan bahwa relay 87L tetap tidak beroperasi pada gangguan eksternal seimbang dalam sistem dengan WT Type 3.
...
Terlihat pada gambar \ref{fig:plot_Ax4_Ay4} arus diferensial dan arus penahan fasa A saat adanya gangguan external 3P menunjukkan bahwa arus berada di bawah batas yang ditentukan oleh setting relay. Relay tidak mendeteksi adanya gangguan maka relay tidak memerintahkan CB untuk trip. Relay 87L beroperasi dengan benar berdasarkan acuan pada Subbab \ref{subsec:tanpa_IBR_ex_3p}.
...
Subbab ini membahas respons relay 87L pada sistem dengan \textit{wind turbine type 3} ketika terjadi gangguan eksternal \textit{single line to ground} (SLG). Gangguan berada di luar daerah proteksi saluran TL89, sehingga relay 87L diharapkan tetap berada pada daerah penahan dan tidak memberikan perintah trip. Hasil pada skenario ini dibandingkan dengan acuan gangguan eksternal SLG tanpa IBR pada Subbab \ref{subsec:tanpa_IBR_ex_slg} untuk menilai apakah integrasi WT Type 3 memengaruhi selektivitas relay diferensial terhadap gangguan tak seimbang.

Analisis dilakukan pada fasa A, fasa B, dan fasa C dengan memperhatikan posisi arus diferensial dan arus penahan terhadap batas operasi relay. Karena gangguan SLG melibatkan satu fasa dan jalur tanah, evaluasi tidak hanya melihat fasa yang terganggu, tetapi juga memastikan fasa lain tetap berada pada daerah penahan. Dengan demikian, pembahasan ini digunakan untuk memastikan bahwa relay 87L tetap mampu membedakan gangguan eksternal tak seimbang dari gangguan internal pada sistem dengan WT Type 3.
...
Terlihat pada gambar \ref{fig:plot_Ax5_Ay5} arus diferensial dan arus penahan fasa A saat adanya gangguan external SLG menunjukkan bahwa arus berada di bawah batas yang ditentukan oleh setting relay. Relay tidak mendeteksi adanya gangguan maka relay tidak memerintahkan CB untuk trip. Relay 87L beroperasi dengan benar berdasarkan acuan pada Subbab \ref{subsec:tanpa_IBR_ex_slg}.
...
Subbab ini membahas respons relay 87L pada sistem dengan \textit{wind turbine type 4} ketika terjadi gangguan eksternal DLG. Titik gangguan berada di luar daerah proteksi saluran TL89, sehingga relay 87L diharapkan tetap berada pada daerah penahan dan tidak memberikan perintah trip. Hasil pada skenario ini dibandingkan dengan acuan gangguan eksternal DLG tanpa IBR pada Subbab \ref{subsec:tanpa_IBR_ex_dlg} untuk menilai apakah karakter kontribusi arus WT Type 4 memengaruhi selektivitas relay diferensial saluran.

Analisis dilakukan pada fasa A, fasa B, dan fasa C melalui hubungan antara arus diferensial dan arus penahan. Karena WT Type 4 memiliki karakter antarmuka konverter yang berbeda dari WT Type 3, pembahasan ini digunakan untuk memastikan bahwa perubahan jenis IBR tidak menyebabkan titik operasi relay melewati batas trip pada gangguan yang berada di luar daerah proteksi. Dengan demikian, hasil subbab ini menjadi pembanding terhadap respons relay 87L pada skenario eksternal DLG dengan WT Type 3.
...
Subbab ini membahas respons relay 87L pada sistem dengan \textit{photovoltaic} ketika terjadi gangguan eksternal DLG. Pada pemodelan yang digunakan, PV tidak memberikan kontribusi arus hubung singkat tambahan karena opsi \textit{No short-circuit contribution} aktif. Oleh karena itu, respons relay pada skenario ini dibaca terutama sebagai akibat konfigurasi jaringan, lokasi gangguan, dan besaran arus yang diterima relay, bukan sebagai akibat injeksi arus gangguan dari sisi PV.

Karena gangguan berada di luar daerah proteksi saluran TL89, relay 87L diharapkan tetap berada pada daerah penahan dan tidak memberikan perintah trip. Hasil pada skenario ini dibandingkan dengan acuan gangguan eksternal DLG tanpa IBR pada Subbab \ref{subsec:tanpa_IBR_ex_dlg}. Analisis dilakukan pada fasa A, fasa B, dan fasa C dengan memperhatikan posisi arus diferensial dan arus penahan terhadap batas operasi relay, sehingga dapat dinilai apakah integrasi PV tetap mempertahankan selektivitas relay diferensial saluran.
